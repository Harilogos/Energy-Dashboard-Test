...
def create_date_filters():
    ...
    yesterday = (datetime.now() - timedelta(days=0)).replace(hour=0, minute=0, second=0, microsecond=0)

    # Calculate min and max values (1 year before and after yesterday)
    min_date = yesterday - timedelta(days=365)
    max_date = yesterday + timedelta(days=365)

    # Add quick date selection buttons
    col1, col2, col3 = st.sidebar.columns(3)

    with col1:
        if st.button("Yesterday", help="Select yesterday's date"):
            st.session_state.date_range = (yesterday, yesterday)

    with col2:
        last_week = yesterday - timedelta(days=7)
        if st.button("Last 7 Days", help="Select the last 7 days"):
            st.session_state.date_range = (last_week, yesterday)

    with col3:
        last_month = yesterday - timedelta(days=30)
        if st.button("Last 30 Days", help="Select the last 30 days"):
            st.session_state.date_range = (last_month, yesterday)
